    ...
    <div class="login">
        <div>
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm"
                     size="small" label-width="66px" label-position="left" class="demo-ruleForm">
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                    name: [
                        { required: true, message: '同学你叫啥？', trigger: 'blur' },
                    ],
                    password: [
                        { required: true, message: '密码来一发', trigger: 'blur' },
                    ],
        ...
        methods: {
            ...
            async login(formName) {
                ...
                if (
                    users.some(
                        item =>
                            item.username === formName.username && item.password === formName.password
                    )
                ) {
                    // 跳转到登录后的页面
                    ...
                    this.$router.replace(
                        this.$route.params.wantedRoute || { name: "content" }
                    );
                } else {
                    alert("账号密码不正确！");
                }
